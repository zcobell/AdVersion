    ...
    for(i=0;i<numPartitions;i++)
    {
        ...
        for(j=0;j<tempMesh->numElements;j++)
        {
            ...
            if(edgeList.contains(e1))
                edgeListDuplicates.append(e1);
            else
                edgeList.append(e1);

            if(edgeList.contains(e2))
                edgeListDuplicates.append(e2);
            else
                edgeList.append(e2);

            if(edgeList.contains(e3))
                edgeListDuplicates.append(e3);
            else
                edgeList.append(e3);

        }

        //...Remove the edges that are part of two elements
        for(j=0;j<edgeListDuplicates.length();j++)
            edgeList.removeOne(edgeListDuplicates.value(j));
...
int Adcirc_hashlib::writePolygonPartitions()
{
    ...
    if(!partitionFile.open(QIODevice::WriteOnly))
        return -1;
    partitionFile.write(QString::number(this->polygons.length()).toUtf8());
    partitionFile.close();

    //...Write polygon files for partition boundaries
    for(i=0;i<this->polygons.length();i++)
    {
        filename.sprintf("%4.4i",i);
        partitionFile.setFileName(this->partitionDir.path()+"/partition_"+filename+".pol");

        if(!partitionFile.open(QIODevice::WriteOnly))
            return -1;

        partitionFile.write(QString(QString::number(this->polygons[i].length())+"\n").toUtf8());
        for(j=0;j<this->polygons[i].length();j++)
        {
            x.sprintf("%+018.12e",this->polygons[i].at(j).x());
            y.sprintf("%+018.12e",this->polygons[i].at(j).y());
            partitionFile.write(QString(x+"    "+y+"\n").toUtf8());
        }
        partitionFile.close();
    }
    return 0;
